...
long query(int node, int s, int e, int i, int j){
    if(s>j || e<i) return 1; //탐색 범위를 벗어나면 0리턴 
    if(s>=i && e<=j) return tree[node]; //탐색 범위에 포함되면 해당 노드의 값 리턴
    int m=s+e>>1; 
    return query(node*2,s,m,i,j)*query(node*2+1,m+1,e,i,j)%1000000007;
}
...
long update(int node, int s, int e, int i, long v){
    ...
    int m=s+e>>1; 
    return tree[node]=update(node*2,s,m,i,v)*update(node*2+1,m+1,e,i,v)%1000000007; 
}
